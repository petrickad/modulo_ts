...
const colegas : string[] = ['petrick', 'andre', 'luan'];
const tecnologias : Array<string> = ['typescript', 'javascript', 'python']; //podemos adicionar e remover elementos com push e pop
const notas: ReadonlyArray<number> = [10, 9, 8, 7, 6]; // não adicionamos ou removemos elementos com push e pop, apenas lemos os elementos

const lista: [nome: string, estaEstudando: boolean, idade: number] = ['petrick', true, 27]; //tupla, podemos adicionar e remover elementos

let idadeDaAna: number | string = 25; 
idadeDaAna = 'vinte e cinco'; // podemos modificar em string e number

let dadosDaApi: any; // qualquer tipo de dado, não é recomendado usar o any, pois não conseguimos fazer validações
